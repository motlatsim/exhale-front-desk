...
exports.handler = async function (event) {
  ...
  try {
    ...
    const q = await notionQueryAll(token, PAYMENTS_DATABASE_ID, { filter: { property: "Family", relation: { contains: familyId } } });
    if (!q.ok) return { statusCode: 200, body: JSON.stringify({ success: true, id: created.id, totalPaid: null }) };

    const totalPaid = q.results.reduce((sum, page) => {
